...
const useRequest = <
  ...
  DataType = ZResponse<PromiseRequestType>,
>(
  ...
  const runComputed = computed(() => {
    ...
    if (option.debounce)
      return {
        ...
        runParams: debounce(runParams, option.throttleInterval) as (p: Params) => Promise<void>,
      }
    // 判断是否开启节流
    if (option.throttle)
      return {
        run: throttle(run, option.throttleInterval) as () => Promise<void>,
        runParams: throttle(runParams, option.throttleInterval) as (p: Params) => Promise<void>,
      }
